...
void plot_HitNoise() {
    ...
    std::vector<std::string> fileNames = {"Plot_allNB_1.root", "Plot_allNB_2.root","Plot_allNB_3.root","Plot_allNB_4.root"};  // Add more files if needed
    ...
    for (size_t i = 0; i < fileNames.size(); i++) {
        ...
        TH1F *HitNoise = (TH1F*) file->Get("HitNoise");
        if (!HitNoise) {
        ...
        TH1F *hist = (TH1F*) HitNoise->Clone(Form("hist_%zu", i));
        hist->Rebin(10);
    ...
    hist->GetXaxis()->SetTitle("Distribution of HitNoise [GeV](Barrel)");
