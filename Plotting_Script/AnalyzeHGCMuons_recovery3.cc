...
int main(int argc, char *argv[]) {
  ...
  bool eta1 = atoi(argv[5]);
  bool eta2 = atoi(argv[6]);
  bool eta3 = atoi(argv[7]);
  bool eta4 = atoi(argv[8]);
  bool eta5 = atoi(argv[9]);

  cout<<massP<<endl;
  AnalyzeHGCMuons_D1 hgcmuons(inputFileList, outFileName, data,massP,eta1,eta2,eta3,eta4,eta5);
  // cout << "dataset " << data << " " << endl;

  hgcmuons.EventLoop(data, eta1, eta2 , eta3, eta4, eta5);
...
void AnalyzeHGCMuons_D1::EventLoop(const char *data, bool eta1, bool eta2, bool eta3, bool eta4, bool eta5) {
  if (fChain == 0)
    return;
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    // Apply barrel or endcap condition to individual electrons
        ...
        if (eta1 && etaValue >= 0 && etaValue <= 1.0) {
            passesEta = true;
        }

        if (eta2 && etaValue >= 1.0 && etaValue <= 1.2) {
            passesEta = true;
        }
        if (eta3 && etaValue >= 1.2 && etaValue <= 1.4) {
            passesEta = true;
        }

        if (eta4 && etaValue >= 1.57 && etaValue <= 2.0) {
            passesEta = true;
        }
        if (eta5 && etaValue >= 2.0 && etaValue <= 2.5) {
            passesEta = true;
        }
