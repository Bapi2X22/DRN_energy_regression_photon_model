...
void plotSigmaOverMuMulti() {
    ...
    for (int f = 0; f < fileNames.size(); ++f) {
        ...
        c_twoGraph->SaveAs(Form("/eos/home-b/bbapi/www/Energy_regression/R9_response_barrel/response_test_%d_barr/BDT_vs_DRN_Barrel_test_%.4f_%.4f.png", (f+1), (double)bin.first, (double)bin.second));
    ...
    for (size_t i = 0; i < graphs.size(); ++i) {
        ...
        if (std::string(yAxis).find("#sigma/#mu") != std::string::npos)
            graphs[i]->GetYaxis()->SetRangeUser(0, 0.03);
        else
    ...
    // TH1F *frame = nullptr;
    if (isMu) {
        TH1F *frame = pad2->DrawFrame(xmin, 0.990, xmax, 1.010);
    } else {
        TH1F *frame = pad2->DrawFrame(xmin, 0.7, xmax, 1.3);
    }
...
plotGraphs(graphs_sigma_BDT, "#sigma/#mu vs R9_Barrel (BDT)", "#sigma/#mu", "sigma_over_mu_vs_R9_BDT_50R_Barrel", false);
plotGraphs(graphs_sigma_DRN, "#sigma/#mu vs R9_Barrel (DRN)", "#sigma/#mu", "sigma_over_mu_vs_R9_DRN_50R_Barrel", false);
plotGraphs(graphs_mu_BDT, "#mu vs R9_Barrel (BDT)", "E_{corr}/E_{gen}", "mu_vs_R9_BDT_50R_Barrel", true);
plotGraphs(graphs_mu_DRN, "#mu vs R9_Barrel (DRN)", "E_{corr}/E_{gen}", "mu_vs_R9_DRN_50R_Barrel", true);
