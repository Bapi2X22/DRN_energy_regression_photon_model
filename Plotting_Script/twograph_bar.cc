...
void superimposePlotsWithFits() {
    ...
    std::vector<std::string> fileNames = {"Plot_barrel_4.root"};
    ...
    lateX->DrawTextNDC(0.19, 0.95, "BDT vs DRN response comparison (Barrel)");
    lateX->SetTextSize(0.03);
    lateX->DrawTextNDC(0.17, 0.55, "pfThresUL18_pedUL18");
    ...
    for (auto fit : fitFuncs_BDT){
        ...
        latex.DrawLatex(0.955, 0.235 + k * 0.02, Form("#mu = %.4f, #sigmaL = %.4f, #sigmaR = %.4f",
                                                     fit->GetParameter(0),
                                                     fit->GetParameter(1),
                                                     fit->GetParameter(2)));
        k++;
    ...
    for (auto fit : fitFuncs_DRN){ 
        ...
        latex.DrawLatex(0.955, 0.235 + (l+1) * 0.02, Form("#mu = %.4f, #sigmaL = %.4f, #sigmaR = %.4f",
                                                     fit->GetParameter(0),
                                                     fit->GetParameter(1),
                                                     fit->GetParameter(2)));
        l++;
    ...
    // Save the canvas
    c_twoGraph->SaveAs("BDT_vs_DRN_comparison(Barrel)_4.pdf");
    c_twoGraph->SaveAs("BDT_vs_DRN_comparison(Barrel)_4.png");
