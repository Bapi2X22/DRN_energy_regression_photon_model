...
{2.0, 2.1}, {2.1, 2.2}, {2.2, 2.3}, {2.3, 2.4},
    ...
    for (int f = 0; f < fileNames.size(); ++f) {
        ...
        c_twoGraph->SaveAs(Form("/eos/home-b/bbapi/www/Energy_regression/eta_response_endcap/response_test_%d_end/BDT_vs_DRN_endcap_test_%.4f_%.4f.png", (f+1), (double)bin.first, (double)bin.second));
        ...
        for (int j = 0; j < nPoints; ++j) {
            ...
            double bin_low = EE_eta_bins[j].first;
            double bin_high = EE_eta_bins[j].second;
    ...
    TLegend *leg = new TLegend(0.12, 0.7, 0.35, 0.88);
    ...
    for (size_t i = 0; i < graphs.size(); ++i) {
        ...
        if (std::string(yAxis).find("#sigma/#mu") != std::string::npos)
            graphs[i]->GetYaxis()->SetRangeUser(0.01, 0.040);
        else
            graphs[i]->GetYaxis()->SetRangeUser(0.98, 1.01);
    ...
    // TH1F *frame = nullptr;
    if (isMu) {
        TH1F *frame = pad2->DrawFrame(xmin, 0.993, xmax, 1.007);
    } else {
...
plotGraphs(graphs_sigma_BDT, "#sigma/#mu vs |#eta|_endcap (BDT)", "#sigma/#mu", "sigma_over_mu_vs_eta_BDT_50R_endcap", false);
plotGraphs(graphs_sigma_DRN, "#sigma/#mu vs |#eta|_endcap (DRN)", "#sigma/#mu", "sigma_over_mu_vs_eta_DRN_50R_endcap", false);
plotGraphs(graphs_mu_BDT, "#mu vs |#eta| (BDT)_endcap", "E_{corr}/E_{gen}", "mu_vs_eta_BDT_50R_endcap", true);
plotGraphs(graphs_mu_DRN, "#mu vs |#eta| (DRN)_endcap", "E_{corr}/E_{gen}", "mu_vs_eta_DRN_50R_endcap", true);
