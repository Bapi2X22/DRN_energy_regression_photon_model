...
void superimposePlotsWithFits() {
    std::vector<std::string> fileNames = {"Plot_barrel_1.root", "Plot_barrel_2.root", "Plot_barrel_3.root", "Plot_barrel_4.root"};
    ...
    std::ofstream outFile("FittedParameters_barrel.txt");
    if (!outFile.is_open()) {
    ...
    lateX->DrawTextNDC(0.19, 0.95, "BDT response for 4 datasets(Barrel)");
    mg_BDT->GetYaxis()->SetRangeUser(0, mg_BDT->GetYaxis()->GetXmax() * 0.25);
    legend_BDT->Draw();
    int k = 0;
    for (auto fit : fitFuncs_BDT){
        ...
        latex.DrawLatex(0.955, 0.22 + k * 0.01, Form("#mu = %.4f, #sigmaL = %.4f, #sigmaR = %.4f",
                                                     fit->GetParameter(0),
                                                     fit->GetParameter(1),
                                                     fit->GetParameter(2)));
        k++;
    };
    c_BDT->SaveAs("BDT_Response_Comparison(Barrel).pdf");
    c_BDT->SaveAs("BDT_Response_Comparison(Barrel).png");
    ...
    // TLatex *lateX = new TLatex();
    lateX->DrawTextNDC(0.25, 0.95, "DRN response for 4 datasets(Barrel)");
    mg_DRN->GetXaxis()->SetTitle("E_{corr}/E_{gen}");
    mg_DRN->GetYaxis()->SetTitle("Normalized");
    mg_DRN->GetYaxis()->SetRangeUser(0, mg_DRN->GetYaxis()->GetXmax() * 0.25);
    legend_DRN->Draw();
    int l = 0; 
    for (auto fit : fitFuncs_DRN){ 
        ...
        latex.DrawLatex(0.955, 0.22 + l * 0.01, Form("#mu = %.4f, #sigmaL = %.4f, #sigmaR = %.4f",
                                                     fit->GetParameter(0),
                                                     fit->GetParameter(1),
                                                     fit->GetParameter(2)));
        l++;
    };
    c_DRN->SaveAs("DRN_Response_Comparison(Barrel).pdf");
    c_DRN->SaveAs("DRN_Response_Comparison(Barrel).png");
}
