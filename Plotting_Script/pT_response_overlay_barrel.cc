...
void plotSigmaOverMuMulti() {
    std::vector<std::string> fileNames = {
        "Plot_final_barrel_1.root",
        "Plot_final_barrel_2.root",
        "Plot_final_barrel_3.root",
        "Plot_final_barrel_4.root"
    };
    ...
    for (int f = 0; f < fileNames.size(); ++f) {
        ...
        std::vector<double> pT_values, sigma_over_mu_BDT, sigma_over_mu_DRN;
        ...
        for (const auto& bin : pT_bins) {
            ...
            for (int iter = 0; iter < 40; ++iter) {
                ...
                bool sigma_converged = std::abs(old_sigma_BDT - sigma_BDT) / sigma_BDT < 0.0002;
            ...
            for (int iter = 0; iter < 40; ++iter) {
                ...
                bool sigma_converged = std::abs(old_sigma_DRN - sigma_DRN) / sigma_DRN < 0.0002;
            ...
            hist_BDT->GetXaxis()->SetRangeUser(xmin, xmax);
            hist_DRN->GetXaxis()->SetRangeUser(xmin, xmax);
            double pT_center = (bin.first + bin.second) / 2.0;
        ...
        c_twoGraph->SaveAs(Form("/eos/home-b/bbapi/www/Energy_regression/pT_response_barrel/response_test_%d_bar_new/BDT_vs_DRN_Barrel_test_%d.png",(f+1) ,(int)bin.first));
            }

        file->Close();

        int nPoints = pT_values.size(); // number of pT bins or eta bins
        ...
        for (int j = 0; j < nPoints; ++j) {
            ...
            double bin_low = pT_bins[j].first;
            double bin_high = pT_bins[j].second;
    ...
    // auto plotGraphs = [&](std::vector<TGraph*> &graphs, const char* title, const char* yAxis, const char* filename) {
    //     TCanvas *c = new TCanvas(filename, title, 800, 600);
    ...
    //         // graphs[i]->GetYaxis()->SetRangeUser(std::string(yAxis).find("#mu") != std::string::npos ? 0.99 : 0, std::string(yAxis).find("#mu") != std::string::npos ? 1.01 : 0.05);
    //     if (std::string(yAxis).find("#sigma/#mu") != std::string::npos) {
    //         graphs[i]->GetYaxis()->SetRangeUser(0, 0.015);
    //     } else {
    //         graphs[i]->GetYaxis()->SetRangeUser(0.99, 1.01);
    //     }            
    //         if (i == 0)
    //             graphs[i]->Draw("APL");
    //         else
    ...
    // plotGraphs(graphs_sigma_BDT, "#sigma/#mu vs pT (BDT)", "#sigma/#mu", "sigma_over_mu_vs_pT_BDT");
    // plotGraphs(graphs_sigma_DRN, "#sigma/#mu vs pT (DRN)", "#sigma/#mu", "sigma_over_mu_vs_pT_DRN");
    // plotGraphs(graphs_mu_BDT, "#mu vs pT (BDT)", "E_{corr}/E_{gen}", "mu_vs_pT_BDT");
    // plotGraphs(graphs_mu_DRN, "#mu vs pT (DRN)", "E_{corr}/E_{gen}", "mu_vs_pT_DRN");
    ...
    for (size_t i = 0; i < graphs.size(); ++i) {
        ...
        // Y-axis range
        if (std::string(yAxis).find("#sigma/#mu") != std::string::npos)
            graphs[i]->GetYaxis()->SetRangeUser(0, 0.02);
        else
            graphs[i]->GetYaxis()->SetRangeUser(0.99, 1.01);
    ...
    TH1F *frame = pad2->DrawFrame(0, 0.9, 310, 1.1);   // Adjust as needed
    // TH1F *frame = nullptr;
    if (isMu) {
        TH1F *frame = pad2->DrawFrame(xmin, 0.996, xmax, 1.004);
    } else {
        TH1F *frame = pad2->DrawFrame(xmin, 0.8, xmax, 1.2);
    }
...
plotGraphs(graphs_sigma_BDT, "#sigma/#mu vs pT_barrel (BDT)", "#sigma/#mu", "sigma_over_mu_vs_pT_BDT_barrel_50R", false);
plotGraphs(graphs_sigma_DRN, "#sigma/#mu vs pT_barrel (DRN)", "#sigma/#mu", "sigma_over_mu_vs_pT_DRN_barrel_50R", false);
plotGraphs(graphs_mu_BDT, "#mu vs pT_barrel (BDT)", "E_{corr}/E_{gen}", "mu_vs_pT_BDT_50R_barrel", true);
plotGraphs(graphs_mu_DRN, "#mu vs pT_barrel (DRN)", "E_{corr}/E_{gen}", "mu_vs_pT_DRN_50R_barrel", true);
