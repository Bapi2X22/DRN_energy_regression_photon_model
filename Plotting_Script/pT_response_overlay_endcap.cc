...
void plotSigmaOverMuMulti() {
    ...
    for (int f = 0; f < fileNames.size(); ++f) {
        ...
        c_twoGraph->SaveAs(Form("/eos/home-b/bbapi/www/Energy_regression/pT_response_endcap/response_test_%d_end/BDT_vs_DRN_endcap_test_%d.png",(f+1) ,(int)bin.first));
            }
    ...
    for (size_t i = 0; i < graphs.size(); ++i) {
        ...
        // Y-axis range
        if (std::string(yAxis).find("#sigma/#mu") != std::string::npos)
            graphs[i]->GetYaxis()->SetRangeUser(0.012, 0.04);
        else
            graphs[i]->GetYaxis()->SetRangeUser(0.97, 1.02);
    ...
    // TH1F *frame = nullptr;
    if (isMu) {
        TH1F *frame = pad2->DrawFrame(xmin, 0.980, xmax, 1.015);
    } else {
        TH1F *frame = pad2->DrawFrame(xmin, 0.8, xmax, 1.27);
    }
...
plotGraphs(graphs_sigma_BDT, "#sigma/#mu vs pT (BDT)_endcap", "#sigma/#mu", "sigma_over_mu_vs_pT_BDT_endcap_50R", false);
plotGraphs(graphs_sigma_DRN, "#sigma/#mu vs pT (DRN)_endcap", "#sigma/#mu", "sigma_over_mu_vs_pT_DRN_endcap_50R", false);
plotGraphs(graphs_mu_BDT, "#mu vs pT (BDT)_endcap", "E_{corr}/E_{gen}", "mu_vs_pT_BDT_endcap_50R", true);
plotGraphs(graphs_mu_DRN, "#mu vs pT (DRN)_endcap", "E_{corr}/E_{gen}", "mu_vs_pT_DRN_endcap_50R", true);
