...
double Cruijff(double *x, double *par) {
    ...
    double xmin = 0.95;
    double xmax = 1.05;
void plotSigmaOverMu(std::vector<std::string> fileNames) {
    // TFile *file = TFile::Open("Plot_R9_endcap_1.root", "READ");
...
{0.87, 0.89}, {0.89, 0.9}, {0.91, 0.92}, {0.92, 0.93},
    ...
    for (const auto &bin : R9_bins) {
        ...
        R9_values.push_back((bin.first + bin.second) / 2.0);
    ...
    file->Close();
            ...
            // Create graphs
    TGraph *graph_sigma_BDT = new TGraph(R9_values.size(), R9_values.data(), sigma_over_mu_BDT.data());
    TGraph *graph_sigma_DRN = new TGraph(R9_values.size(), R9_values.data(), sigma_over_mu_DRN.data());

    TGraph *graph_mu_BDT = new TGraph(R9_values.size(), R9_values.data(), mu_hist_BDT.data());
    TGraph *graph_mu_DRN = new TGraph(R9_values.size(), R9_values.data(), mu_hist_DRN.data());
    ...
    TCanvas *c = new TCanvas("c", "Sigma/mu vs R9", 800, 600);
    // c->SetLogy();
    graph_sigma_BDT->Draw("APL");
    graph_sigma_DRN->Draw("PL same");

    graph_sigma_BDT->SetTitle("#sigma/#mu vs R9(barrel)");
    graph_sigma_BDT->GetXaxis()->SetTitle("R9");
    ...
    std::string plotName1 = "sigma_over_mu_plot_" + fileName + ".png";
    std::string plotName2 = "sigma_over_mu_plot_" + fileName + ".pdf";

    c->SaveAs(plotName1.c_str());
    c->SaveAs(plotName2.c_str());

    TCanvas *c1 = new TCanvas("c1", "mu vs R9", 800, 600);
    ...
    graph_mu_BDT->GetXaxis()->SetTitle("R9");
    ...
    std::string plotName3 = "mu_plot_" + fileName + ".png";
    std::string plotName4 = "mu_plot_" + fileName + ".pdf";

    c1->SaveAs(plotName3.c_str());
    c1->SaveAs(plotName4.c_str());
    ...
    // graph_sigma_BDT->SetTitle("#sigma/#mu vs R9(Endcap)");
    // graph_sigma_BDT->GetXaxis()->SetTitle("R9");
    // graph_sigma_BDT->GetYaxis()->SetTitle("#sigma/#mu");
    // graph_sigma_BDT->GetYaxis()->SetRangeUser(0, 0.05);
    ...
    // c->SaveAs("sigma_over_mu_vs_R9_1_endcap.png");
    // c->SaveAs("sigma_over_mu_vs_R9_1_endcap.pdf");
    ...
    // graph_mu_BDT->SetTitle("#mu vs pT(Endcap)");
    // graph_mu_BDT->GetXaxis()->SetTitle("R9");
    // graph_mu_BDT->GetYaxis()->SetTitle("E_{corr}/E_{gen}");
    // graph_mu_BDT->GetYaxis()->SetRangeUser(0.99, 1.01);
    ...
    // c1->SaveAs("mu_vs_R9_1_Endcap.pdf");
    // c1->SaveAs("mu_vs_R9_1_Endcap.png");

}

int main() {
    std::vector<std::string> fileNames = {"Plot_R9_barrel_1.root", "Plot_R9_barrel_2.root", "Plot_R9_barrel_3.root", "Plot_R9_barrel_4.root"};
    plotSigmaOverMu(fileNames);
    return 0;
}
